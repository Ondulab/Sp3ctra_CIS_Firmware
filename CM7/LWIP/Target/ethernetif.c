...
#define INTERFACE_THREAD_STACK_SIZE ( 2048 )   /* bytes (CMSIS-RTOS v2): was 350 words in v1 */
...
/* USER CODE BEGIN 1 */
/* CubeMX 6.18.1 / FW_H7 1.13.0 template: low_level_init() still initialises a global
 * TxConfig that the template no longer declares (TX path now uses a local tx_config). */
...
/* USER CODE END OS_THREAD_ATTR_CMSIS_RTOS_V2 */
  ...
  /* Start ETH HAL Init */
   ...
   uint8_t MACAddr[6] ;
  heth.Instance = ETH;
  sys_identity_mac(MACAddr);   /* per-unit locally administered MAC (see post_cubemx_restore.sh) */
