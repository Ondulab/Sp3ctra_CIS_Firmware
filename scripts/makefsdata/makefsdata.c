...
/*
 * NOTE (host build): This tool is compiled as a *host* executable.
 * We only need a small subset of lwIP types/macros at build time.
 * Pulling the full STM32 project configuration (lwipopts.h -> main.h -> HAL)
 * makes host compilation painful. Therefore we provide minimal stand-ins.
 */
...
#ifndef LWIP_ASSERT
#define LWIP_ASSERT(message, assertion) do { (void)(message); if (!(assertion)) { abort(); } } while (0)
#endif

/* defaults used by checksum precompute path; can be overridden at compile time */
#ifndef TCP_MSS
#define TCP_MSS 1460
#endif
...
/*
 * Minimal HTTPD tables required by makefsdata:
 * Avoid including lwIP's "httpd_structs.h" because it drags in lwIP options
 * and the embedded project configuration.
 */
#define NUM_SHTML_EXTENSIONS 3
static const char *g_pcSSIExtensions[NUM_SHTML_EXTENSIONS] = {
  ".shtml",
  ".shtm",
  ".ssi"
};
...
/* keep in sync with response_type mapping below */
enum {
  HTTP_HDR_OK = 0,
  HTTP_HDR_NOT_FOUND,
  HTTP_HDR_BAD_REQUEST,
  HTTP_HDR_NOT_IMPL,
  HTTP_HDR_OK_11,
  HTTP_HDR_NOT_FOUND_11,
  HTTP_HDR_BAD_REQUEST_11,
  HTTP_HDR_NOT_IMPL_11,
  HTTP_HDR_CONTENT_LENGTH,
  HTTP_HDR_CONN_KEEPALIVE,
  HTTP_HDR_CONN_CLOSE,
  HTTP_HDR_MAX
};

static const char *g_psHTTPHeaderStrings[HTTP_HDR_MAX] = {
  /* 0 */ "HTTP/1.0 200 OK\r\n",
  /* 1 */ "HTTP/1.0 404 File not found\r\n",
  /* 2 */ "HTTP/1.0 400 Bad Request\r\n",
  /* 3 */ "HTTP/1.0 501 Not Implemented\r\n",
  /* 4 */ "HTTP/1.1 200 OK\r\n",
  /* 5 */ "HTTP/1.1 404 File not found\r\n",
  /* 6 */ "HTTP/1.1 400 Bad Request\r\n",
  /* 7 */ "HTTP/1.1 501 Not Implemented\r\n",
  /* 8 */ "Content-Length: ",
  /* 9 */ "Connection: keep-alive\r\n",
  /* 10 */ "Connection: close\r\n"
};

/* MIME types (must include the final double CRLF) */
...
#define NUM_HTTP_HEADERS (sizeof(g_psHTTPHeaders) / sizeof(g_psHTTPHeaders[0]))

/* file flags (subset from lwIP httpd/fs.h) */
...
#define FS_FILE_FLAGS_HEADER_HTTPVER_1_1   0x04U
#endif
#ifndef FS_FILE_FLAGS_SSI
#define FS_FILE_FLAGS_SSI                  0x08U
#endif

/*
 * The generator uses inet_chksum() and lwip_itoa().
 * We include the lwIP implementation .c files directly (same as original).
 */
